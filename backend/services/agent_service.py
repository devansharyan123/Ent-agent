# backend/services/agent_service.py - Add recommendation tool integration
...
class AgentService:
    ...
    def execute_query(
        ...
        try:
            ...
            recommendations = ""
            try:
                rec_result = recommendation_tool(
                    query=query,
                    user_role=user_role,
                    conversation_id=str(conversation_id),
                )
                recommendations = rec_result.get("answer", "")
                # Merge sources from recommendation tool if needed
                if rec_result.get("sources"):
                    # Add unique sources from recommendations
                    existing_sources = {(s["file_name"], s["category"]) for s in sources}
                    for rec_src in rec_result["sources"]:
                        key = (rec_src["file_name"], rec_src["category"])
                        if key not in existing_sources:
                            sources.append(rec_src)
            except Exception as rec_exc:
                logger.warning(f"Recommendation generation failed: {rec_exc}")
                recommendations = "- Would you like to know more about related policies?"
    ...
    def get_documents(user_role: str, db: Session = None) -> List[Dict[str, Any]]:
        ...
        role_categories: Dict[str, List[str]] = {
            "admin":    ["admin", "hr", "general", "payroll", "leave"],
            "hr":       ["hr", "general", "payroll", "leave"],
            "employee": ["general", "leave", "payroll"],
        }
